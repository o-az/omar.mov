...
export function CommentsSection(props: { slug: string }) {
  ...
  const label = import.meta.env.VITE_UTTERANCES_LABEL
  const theme = import.meta.env.VITE_UTTERANCES_THEME

  if (!repo) {
    return import.meta.env.DEV ? (
      <section class='max-w-170 mx-auto mt-4 pt-2 border-t border-slate-200/70 dark:border-slate-800 text-sm text-slate-500'>
        Set <code class='font-mono'>VITE_UTTERANCES_REPO</code> to enable comments.
      </section>
    ) : null
  }
